...
/**
 * Provide a GUI view of a SchoolDirectory.
 * Different panes provide access to the data in the directory.
 *
 *      One to search the address book.
 *
 *      One to allow a set of student details to be entered.
...
public class SchoolDirectoryGUI extends JFrame {
	/**
	 * 
	 */
	private static final long serialVersionUID = 9072027243560165906L;
	 ...
	 * @param dir The directory to be manipulated.
	 */
	public SchoolDirectoryGUI(SchoolDirectory dir)
	{
		this.dir = dir;
		setTitle("School Directory");
		...
		contentPane.add(tabbedArea, BorderLayout.CENTER);
		
		ImageIcon icon = null;
		String imageName = "hogwartsCrest.jpg";
	...
	private Container setupDataEntry()
	{
		...
		// Set up the year area.
		Box yearLabelArea = Box.createHorizontalBox();
		yearLabelArea.add(new JLabel("Year", JLabel.LEFT));
		yearLabelArea.add(Box.createGlue());
		final JTextField yearField = new JTextField(50);
		Box yearArea = Box.createVerticalBox();
		yearArea.add(yearLabelArea);
		yearArea.add(yearField);
		...
		add.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev)
			{
				...
				if (isYearSet) {
				dir.enrollStudent(new Student1(
								nameField.getText(), year));
				}
				else {
					dir.enrollStudent(new Student1(nameField.getText()));
				}
			}
		});
		...
		searchField.getDocument().addDocumentListener(new DocumentListener(){
			...
			private void research()
			{
				...
				if(searchString.length() > 0) {
					ArrayList<Student1> results = dir.searchByKey(searchString);
					for(Student1 student : results) {
